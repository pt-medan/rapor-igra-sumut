        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('guru.kelompok-kelas.update', $kelompokKelas) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="space-y-4">
                            <div>
                                <x-input-label for="nama_kelompok" :value="__('Nama Kelas')" />
                                <x-text-input id="nama_kelompok" class="block mt-1 w-full" type="text" name="nama_kelompok" :value="old('nama_kelompok', $kelompokKelas->nama_kelompok)" required autofocus />
                                <x-input-error :messages="$errors->get('nama_kelompok')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="tahun_ajaran" :value="__('Tahun Ajaran')" />
                                <x-text-input id="tahun_ajaran" class="block mt-1 w-full bg-gray-100" type="text" name="tahun_ajaran" :value="old('tahun_ajaran', $kelompokKelas->tahun_ajaran)" readonly />
